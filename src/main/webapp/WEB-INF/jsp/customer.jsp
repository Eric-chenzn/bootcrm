				...
				<form class="form-inline" method="get"
					...
					<div class="form-group">
						...
						<select	class="form-control" id="customerFrom" name="custSource">
							...
							<c:forEach var="item" items="${fromType}">
					...
					<div class="form-group">
						...
						<select	class="form-control" id="custLevel" name="custLevel">
							<option value="">--请选择--</option>
							<c:forEach var="item" items="${levelType}">
								<option value="${item.dict_id}"
		...
		<div class="row">
			<div class="col-lg-12">
				<div class="panel panel-default">
							...
							<c:forEach items="${customerList}" var="row">
								<tr>
									...
									<td>
										...
										<a href="#" class="btn btn-danger btn-xs" data-toggle="modal" onclick="deleteCustomer(${row.cust_id})" >删除</a>
									</td>
					...
					<div class="col-md-12 text-right" id="pageNavigation">
...
<script type="text/javascript">
	...
	function deleteCustomer(id) {
		if (confirm("您确定要删除该客户的信息吗？")){
			$.post("<%=basePath%>/customer/deleteCustomerById.action",{"id":id},function (data) {
				if(data=="OK"){
					alert("客户信息删除成功!");
					window.location.reload();
				}else{
	...
	//创建分页栏
	function createNavigation(rows,page,customerCount) {
	    console.log(rows);
	    console.log(page);
		...
		var number=5;
	    //计算总页数
		var  pageCount=parseInt(customerCount/rows + "");
		if (customerCount%rows>0){
			pageCount++;
		}
		console.log(pageCount);

		var searchUrl="&custName=${custName}&custSource=${custSource}&custIndustry=${custIndustry}&custLevel=${custLevel}";
		...
		//首页、上一页
		var homeUrl="<%=basePath%>customer/list.action?page=1&&rows"+rows+searchUrl;
		var preUrl="<%=basePath%>customer/list.action?page="+(page-1)+"&&rows"+rows+searchUrl;
		var homePage=$("<li><a href="+homeUrl+">首页</a></li>");
		var prePage=$("<li><a href="+preUrl+">上一页</a></li>");
		if (page<=1){
			homePage.addClass("disabled");
			prePage.addClass("disabled");
		}
		navUl.append(homePage);
		navUl.append(prePage);
		...
		//页码设置
		for (var i=1;i<=number&&pageIndex<=pageCount;pageIndex++,i++) {
			var pageUrl="<%=basePath%>customer/list.action?page="+pageIndex+"&&rows="+rows+searchUrl;
			var pageCode1=$("<li><a href="+pageUrl+">"+pageIndex+"</a></li>");
			if (pageIndex==page){
					pageCode1.addClass("active");
				var sp=$("<span class=\'sr-only\'>(current)</span>");
					pageCode1.children("a").append(sp);
			}
			navUl.append(pageCode1);
		}

		//下一页、尾页

		var nextUrl="<%=basePath%>customer/list.action?page="+(page+1)+"&&rows"+rows+searchUrl;
		var finalUrl="<%=basePath%>customer/list.action?page="+pageCount+"&&rows"+rows+searchUrl;
		var nextPage=$("<li><a href="+nextUrl+">下一页</a></li>");
		var finalPage=$("<li><a href="+finalUrl+">尾页</a></li>");
		if (page>=pageCount){
			nextPage.addClass("disabled");
			finalPage.addClass("disabled");
		}
		navUl.append(nextPage);
		navUl.append(finalPage);


		//将分页栏添加到导航栏
		$("#pageNavigation").append(nav);

	}
	createNavigation(${rows},${page},${customerCount});
</script>
</body>
</html>